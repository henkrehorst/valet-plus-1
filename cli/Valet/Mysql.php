...
class Mysql
{
    ...
    {
        $this->files->unlink($this->brew->getBrewPrefix() . static::MYSQL_CONF);
        $this->files->unlink($this->brew->getBrewPrefix() . static::MYSQL_CONF . '.default');
    }
    ...
    public function installConfiguration($type = 'mysql@5.7')
    {
        ...
        $this->files->chmodPath($this->brew->getBrewPrefix() . static::MYSQL_DIR, 0777);

        if (!$this->files->isDir($directory = $this->brew->getBrewPrefix() . static::MYSQL_CONF_DIR)) {
        ...
        $this->files->putAsUser(
            $this->brew->getBrewPrefix() . static::MYSQL_CONF,
